...
export default function ActiveWorkoutScreen() {
      ...
      if (Array.isArray(fieldErrors)) {
        fieldErrors.forEach((error: string) => {
          ...
          if (error.includes('less than or equal to 100')) {
            friendlyMessage =
              field === 'reps' ? 'Reps must be between 0 and 100' : 'RIR must be between 0 and 100';
  ...
  const handleAddSet = async (
    ...
    set: { weight: number; reps: number; reps_in_reserve?: number }
  ) => {
    try {
  ...
  const handleFinishWorkout = async (options?: {
    ...
    // If options are already provided (e.g., from a recursive call, though we handle it via closure above), skip the initial confirmation alert.
    if (options && (options.normalize_duration || options.proceed_as_is)) {
      await executeCompletion(options);
      return;
    }
    ...
    Alert.alert('Finish Workout', 'Are you sure you want to finish this workout?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Finish',
        style: 'default',
        onPress: () => executeCompletion(options),
      },
    ]);
  };
  ...
  const totalSets =
    activeWorkout?.exercises?.reduce((acc: number, ex: any) => acc + (ex.sets?.length || 0), 0) ||
    0;
