      ...
      <View style={trainingIntensityStyles.upperSection}>
        <View style={trainingIntensityStyles.upperLeft}>
          <View style={trainingIntensityStyles.intensityBars}>
            {[0.2, 0.2, 0.2].map((opacity, index) => (
              <View
                key={index}
                style={[
                  trainingIntensityStyles.bar,
                  { opacity, backgroundColor: theme.colors.status.rest },
                ]}
              />
            ))}
          </View>
          <View style={trainingIntensityStyles.intensityTextContainer}>
            ...
            <Text
              style={[trainingIntensityStyles.intensityValue, { color: theme.colors.status.rest }]}
            >
              RECOVERY
            </Text>
          </View>
        ...
        <View
          style={[
            trainingIntensityStyles.intensityIcon,
            {
...
export default function WorkoutModal({ visible, onClose, mode, onSuccess }: WorkoutModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload =
        mode === 'log'
          ? { title: workoutTitle, date: formatLocalISO(date), is_done: true }
          : { title: workoutTitle, date: formatLocalISO(new Date()) };

      const result = await createWorkoutMutation.mutateAsync(payload);
      ...
      const err = error as {
        response?: { status: number; json: () => Promise<{ error?: string; message?: string }> };
      };
      ...
      >
        ...
        <View style={styles.sheetContainer}>
          ...
          <View style={styles.formContainer}>
            ...
            >
              <Ionicons
                name={mode === 'create' ? 'play' : 'checkmark'}
                size={18}
                color={theme.colors.text.primary}
              />
